define([
    ...
    var ServerReservedEventFormView = Backbone.View.extend({
        ...
        setDropList: function() {
            var t = this;
            t.serverReservedEventType = new DropButtonView({
                el: t.$el.find('.serverReservedEventType')
            }, {
                containerId: 'serverReservedEventType',
                fieldName: 'target'
            });
            var defultPageType = 'element';
            t.serverReservedEventType.setListData(Dictionary.ServerReservedEvent, defultPageType);
        },
        //清空表单的数据
        initForm: function() {
            var t = this;
        ...
        saveFormEvent: function() {
            ...
            //updating
            if (update) {
                var confirmContnet = '是否更新服务器保留事件?';
                if (config.formType == 'add') {
                    confirmContnet = '是否添加服务器保留事件?';
                }
                new Modal.Confirm({
                    title: '提示',
    ...
    return ServerReservedEventFormView;
});